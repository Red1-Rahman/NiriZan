#!/usr/bin/env bash
# NiriZan dependency graph generator (macOS, BSD tools, bash 3.2 compatible).
...
# (CI uses 3.11). The system python3 from the Xcode command line tools is
# usually older, so also try the usual Homebrew locations, which are not
# always on PATH in non-login shells.
PYTHON=""
