...
(workflow_dispatch) trigger only, or locally through the thin wrappers in
console/linux/, console/macOS/ and console/windows/. The workflow opens a PR
with the result; this script never commits, pushes, or touches git at all.
...
# Same interpreter version the GitHub Actions workflow uses.
MIN_PYTHON = (3, 11)

# Single source of truth for the notice at the top of the generated file.
HEADER_LINES = (
    "> Generated by `.github/scripts/gen_dependency_graph.py`.",
    "> Regenerate locally with the script in `console/linux/`, `console/macOS/`,",
    "> or `console/windows/`, or through the `dependency-graph` GitHub Actions",
    ...
    "> will be overwritten by the next generation.",
)
...
def main() -> None:
    ...
    if sys.version_info < MIN_PYTHON:
        ...
        sys.exit(f"error: Python {required} or newer is required (found {found}).")
    ...
    # Explicit UTF-8 and LF so the output is byte-identical on Windows, Linux,
    # macOS and CI (Path.write_text(newline=...) needs Python 3.10+).
