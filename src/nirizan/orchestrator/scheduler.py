...
from nirizan.storage.run_repository import RunRepository
...
class TraceSource(Protocol):
    """The shape RunScheduler needs from a repository: read-only trace lookup by app name."""

    async def list_by_application(self, application_name: str, limit: int = 100) -> list[Trace]: ...


class RunScheduler:
    """Triggers an evaluation run on-demand: traces -> dispatch -> Run, persisted via RunRepository."""
    ...
    async def run_on_demand(self, application_name: str, system_type: str) -> list[Run]:
