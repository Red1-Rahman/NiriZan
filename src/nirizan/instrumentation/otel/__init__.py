...
Import Hygiene:
    ...
    is not installed in the target environment, this `__init__.py` only re-exports
    `_id_mapping` and `semconv`. Modules requiring OpenTelemetry packages (`to_otel`
    and `from_otel`) must be explicitly imported by callers.
"""
