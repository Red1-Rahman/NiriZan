...
def _get_default_tracer() -> Tracer:
    ...
    return trace.get_tracer("nirizan", instrumenting_library_version=version)
...
def convert_span_to_otel_attributes(
    ...
    attributes[NIRIZAN_SPAN_ID_SOURCE] = getattr(
        span, "span_id_source", SPAN_ID_SOURCE_ROUNDTRIP
    )
    ...
    if kind_upper in ("GENERATION", "LLM", "CHAT"):
        ...
        if output_payload is not None:
            attributes[NIRIZAN_RETRIEVAL_RESULTS] = _format_payload_value(
                output_payload
            )
    ...
    if isinstance(custom_attrs, dict):
        for k, v in custom_attrs.items():
            ...
            if isinstance(v, (int, float, bool)):
                ...
                try:
                    attributes[k] = truncate_attribute_value(
    ...
    if hasattr(span, "exception"):
        ...
        if exc is not None:
            if isinstance(exc, BaseException):
                ...
                otel_span.add_event(
                    "exception", attributes={"exception.message": str(exc)}
                )
...
class NiriZanToOTelExporter(BaseExporter):
    ...
    def export_trace(
        self, trace_obj: Trace | Sequence[Span] | Any
    ) -> list[trace.Span]:
        """Export a full NiriZan trace structure to OpenTelemetry."""
        return export_trace_to_otel(trace_obj, tracer=self._tracer)"""OpenTelemetry trace exporter for NiriZan.
