...
NiriZan is an independent open-source project. The latest released version is `0.4.0`, (see `CHANGELOG.md`). The package is publicly available on PyPI and can be installed with:
...
NiriZan currently has a single branch, `main`. There are no separate long-term maintenance or long-term-support branches.

The latest released version is `0.4.0` (released 2026-09-20). The `CHANGELOG.md` is the authoritative record of NiriZan's released versions and changes.

`main` may contain changes that have not yet been released. Security fixes are developed on `main` and incorporated into subsequent releases as appropriate.

| Version / Branch                      | Supported                         |
| ------------------------------------- | --------------------------------- |
| `main` (including unreleased changes) | Yes, best effort                  |
| `0.4.0` (latest released version)     | Yes, best effort                  |
