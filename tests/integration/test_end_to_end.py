...
@pytest.fixture
async def trace_repo(db_path: str) -> AsyncGenerator[SQLiteTraceRepository, None]:
...
) -> AsyncGenerator[TraceCollector, None]:
...
async def test_end_to_end_phase4_statistical_gating_and_ci_summary(
    ...
    assert trace_b.code_commit == "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
    ...
    with caplog.at_level(logging.INFO):
        ...
        # --- Optional structure track through gate, CI, and dashboard ---
        structure_comparator = MultivariateComparator(
        ...
        structure_baseline = ScoreMatrix(
            values=np.column_stack(
                [
                    baseline_scores["context_relevance"],
                    baseline_scores["groundedness"],
                    baseline_scores["answer_relevance"],
                ]
            ),
            metric_names=("context_relevance", "groundedness", "answer_relevance"),
        )
        structure_candidate = ScoreMatrix(
            values=np.column_stack(
                [
                    candidate_block_scores["context_relevance"],
                    candidate_block_scores["groundedness"],
                    candidate_block_scores["answer_relevance"],
                ]
            ),
            metric_names=("context_relevance", "groundedness", "answer_relevance"),
        )
        multivariate_verdicts = structure_comparator.compare(
            candidate=structure_candidate,
            baseline=structure_baseline,
            baseline_id=baseline_id,
            run_id=run_b.run_id,
        )

        block_gate_with_structure = evaluate_gate(
            verdicts=block_verdicts,
            scores_by_metric=block_scores_by_metric,
            multivariate_verdicts=multivariate_verdicts,
        )
        assert block_gate_with_structure.multivariate_verdicts == multivariate_verdicts
        assert block_gate_with_structure.passed is False

        dashboard_snapshot = assemble_dashboard_snapshot(
            system_type=sys_type,
            quality_score=0.35,
            confidence=0.90,
            regression_verdicts=block_verdicts,
            multivariate_verdicts=multivariate_verdicts,
            gate_verdict=block_gate_with_structure,
        )
        assert dashboard_snapshot.gate_verdict == block_gate_with_structure
        assert dashboard_snapshot.multivariate_verdicts == multivariate_verdicts
        ...
        assert "Structure track (multivariate):" in summary_text
...
async def test_end_to_end_trust_attribution_and_dashboard_reporting(
    ...
    2. Aggregate multi-outcome verdict windows with compute_judge_reliability().
     3. Verify judge reliability status flags UNSTABLE when judge drift +
         joint drift rate exceeds threshold.
     4. Assemble full DashboardSnapshots and assert system health score
         penalties for JOINT_DRIFT and INCONCLUSIVE.
